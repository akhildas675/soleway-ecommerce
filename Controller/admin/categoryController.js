...
const categoryAdding = async (req, res) => {
    try {
        ...
        const categoryExisting = await Category.findOne({ categoryName: { $regex: new RegExp('^' + checkCategory + '$', 'i') } });
...
const editingCategory= async (req,res)=>{
    try {
        ...
        const categoryExisting = await Category.findOne({_id: { $ne: req.session.categoryId}, categoryName: { $regex: new RegExp('^' + checkCategory + '$', 'i') } });
